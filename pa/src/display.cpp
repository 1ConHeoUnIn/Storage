...
void oled_display::first_scan_view(int heading, int rssi, int best_rssi, int best_heading, int spin)
{
    draw_compass(heading);
    show_rssi(rssi);
    show_best_rssi(best_rssi);
...
void oled_display::show_spin(int spin)
{
  
    char buffer[10];
    snprintf(buffer, sizeof(buffer),"spin: %d", spin);
    u8g2.drawStr(5, 55, buffer);
    u8g2.sendBuffer();
}
